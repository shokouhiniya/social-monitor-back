...
async function seed() {
  ...
  const alertsData = [
    ...
    { title: 'فرصت: واکنش سناتور استرالیایی', message: 'سارا هانسون در مجلس استرالیا از فلسطین حمایت کرد. فرصت طلایی برای تعامل و ارسال محتوا. تیم تعامل بین‌الملل فعال شود.', priority: 'high', category: 'opportunity', target_pages: [pages[3].id], status: 'active', created_by: users[1].id },
    { title: 'بحران: آتش‌سوزی سلماس', message: 'آتش‌سوزی بزرگ در سلماس رخ داده و پیج محلی در حال انتشار تبلیغات است. واکنش فوری لازم است. دستور توقف تبلیغات صادر شود.', priority: 'critical', category: 'crisis', target_pages: [pages[2].id], status: 'active', created_by: users[0].id },
    { title: 'تغییر ترند: نفوذ در لایه خاکستری', message: 'پیج لایف‌استایل مدرن با انتشار محتوای سیاسی بازخورد بالایی گرفت. این الگو قابل تکرار در سایر پیج‌های لایف‌استایل است.', priority: 'medium', category: 'trend_shift', target_pages: [pages[10].id], status: 'active', created_by: users[2].id },
    { title: 'هشدار: کاهش فعالیت کانال دانشجویی', message: 'کانال دانشجوی آزاد فعالیتش به شدت کاهش یافته. بررسی فوری وضعیت ادمین و در صورت نیاز فعال‌سازی جایگزین.', priority: 'high', category: 'crisis', target_pages: [pages[15].id], status: 'active', created_by: users[0].id },
  ];
